...
public class solutionUsingPattern50+ 
{
    ...
    public static void setup(int length, int a, int b, int c, int d, int e)
    {
        int evens = 1;
        int odds = 1;
        ...
        rule(length, a, b, c, d, e);

        
    }

    public static void rule(int length, int a, int b, int c, int d, int e)
    {
        int y = length/mod;
        ...
        if(delete(temp) != -1)
        {
            if((temp+2) < length && solution.get(temp+1) != -1)
            {
                insert(temp+2, e);
            } else {
                insert(temp-2, e);
